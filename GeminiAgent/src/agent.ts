...
export const wordQuizQuestionSchema = z.object({
  question: z.string().describe('The quiz question text'),
  options: z.array(z.object({
    id: z.enum(['A', 'B', 'C', 'D']).describe('The option identifier'),
    text: z.string().describe('The option text'),
  })).length(4).describe('Exactly 4 multiple-choice options'),
  correct_answer: z.enum(['A', 'B', 'C', 'D']).describe('The correct answer option ID'),
  type: z.enum(['meaning', 'usage', 'synonym', 'antonym', 'fillBlank']).describe('The question type'),
  about: z.string().describe('The Thai word being tested'),
});

export const wordQuizListSchema = z.object({
  quizzes: z.array(wordQuizQuestionSchema).describe('The list of all generated quizzes, shuffled'),
});

export const WORD_QUIZ_PROMPT = `Generate a complete set of Thai vocabulary exercises in JSON format. For each given Thai word/phrase, create five multiple-choice questions covering meaning, usage, synonym, antonym, and a fill-in-the-blank context.
